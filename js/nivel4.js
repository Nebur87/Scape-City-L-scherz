...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (confirmBtn) {
    confirmBtn.onclick = function() {
      ...
      if (foundClueBtn) foundClueBtn.style.display = 'inline-block';
  ...
  if (foundClueBtn) {
    foundClueBtn.onclick = function() {
      ...
      this.style.display = 'none';
      if (readyNextBtn) readyNextBtn.style.display = 'inline-block';
        ...
        if (!window.qrScannerStarted) {
          ...
          const qrCodeSuccessCallback = (decodedText, decodedResult) => {
            if (decodedText === 'nivel5.html') {
              if (qrResults) qrResults.textContent = '¡QR correcto! Avanzando al siguiente nivel...';
              setTimeout(() => {
                window.location.href = 'nivel5.html';
              }, 1200);
            } else {
              if (qrResults) qrResults.textContent = 'QR no válido para este nivel.';
            }
          };
          if (window.Html5Qrcode) {
            ...
            if (qrResults) qrResults.textContent = 'No se pudo cargar el lector QR.';
          }
        }
      } else {
